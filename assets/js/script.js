...
let ausgabe = document.getElementById("content");

// h1
let h1 = document.createElement("h1");
h1.textContent = "Zufälliges Bild generieren";
ausgabe.appendChild(h1);

// button
let button = document.createElement("button");
button.textContent = "KlickediKlick";
...
// quelle
let quelle = document.createElement("a");
// Funktion
button.addEventListener("click", () => {
	let generator = Math.floor(Math.random() * data.length);
	img.src = data[generator].download_url;
	ausgabe.appendChild(img);
	console.log(img.src);
	let url = "url(" + img.src + ")";
	...
	autorDisplay = data[generator].author;
	autor.textContent = autorDisplay;
	div.appendChild(autor);

	quelle.href = data[generator].url;
	quelle.textContent = quelle.href;
	div.appendChild(quelle);
});
